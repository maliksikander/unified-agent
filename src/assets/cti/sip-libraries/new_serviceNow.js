...
if (window.addEventListener) {
	...
	window.addEventListener("message", function (e) {
		...
		//new code jazeb
		if(e.data.event == "newInboundCall"){
			console.log("ringing call event data ",e.data);
            let message = {
                phoneno : e.data.response.dialog.ani,	
				myevent:"Ringing",
            }
	window.parent.postMessage(message,"*");
		}
		else if(e.data.event == "dialogState"){
			console.log(" call end event data ",e.data);
		}
...
//-----------------------------------------------------------------------------
 function createActivity(callerNumber, duration, callType, description, call) {
     try {
		 ...
		 if(call.response.dialog.callType == "CONFERENCE" && call.response.dialog.state == "DROPPED" && call.response.dialog.participants.Participant.length !== undefined)
		 {return;}
		 if(call.response.dialog.callType == callTypes.silentMonitorType)
		 {return;}
